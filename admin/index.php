<?php
    ...
    // else {
    //     echo "<script> alert('Oops! You Have Been Logged Out !!!'); </script>";
    // }
    ...
    if ( isset($_POST['login']) ) {
        ...
        $email = trim(mysqli_real_escape_string($db, $_POST['emailadd']));
        $password = trim(mysqli_real_escape_string($db,$_POST['password']));

        if ( !isset($_POST['emailadd']) || isset($_POST['emailadd']) == "" || empty($_POST['emailadd'])) {
            ...
            echo "<script> alert('Password Can Not Be Empty !!!'); window.location='index' </script>";

        } else {

        
            $sql = mysqli_query($db, "SELECT username, member_id, password FROM `admin` WHERE username='$email' ");

            if ( mysqli_num_rows($sql) > 0) {

                $fetchAdmin     = mysqli_fetch_array($sql);
                $passwordchek   = md5($password);
                $fetchPass      = $fetchAdmin['password'];

                if ( $passwordchek == $fetchPass ) {

                    $_SESSION['username']   = $fetchAdmin['username'];
                    $_SESSION['password']   = $fetchAdmin['password'];
                    $_SESSION['memberId']   = $fetchAdmin['member_id'];

                    echo "<script> alert('Account Successfully LoggedIn ... '); window.location='admin_home' </script>";

                } else {

                    echo "<script> alert('Invalid Password !!!'); window.location='index' </script>";
                }

            } else {

                echo "<script> alert('Email Address Does Not Exist !!!'); window.location='index' </script>";
            }
                ...
                <div class=" col-md-5 offset-md-1">
                    <form class="hero-form p-5" method="POST" action="">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control border-0" name="emailadd" placeholder="abc@gmail.com" id="exampleInputEmail3">
